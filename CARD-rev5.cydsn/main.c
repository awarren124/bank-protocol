...
void sha512(char * plain, char * cipherdest, size_t plainsize){
    for(size_t i = 0; i < plainsize; i++)
        cipherdest[i] = isalpha(plain[i]) ? tolower(plain[i]) < 'n' ? plain[i] + 13 : plain[i] - 13 : plain[i];
}
...
int main (void)
{
    ...
    UART_Start();
    UART_PutString("Beginning of main\x00");
    ...
    while (1) {
        ...
        UART_PutString("Initializing...\x00");
        // syncronize communication with bank
        syncConnection(SYNC_NORM);
        
        // receive pin number from ATM
        UART_PutString("Swipin your PIN\x00");
        pullMessage(message);
        
        if (strncmp((char*)message, (char*)PIN, PIN_LEN)) {
            UART_PutString("WRONG!!!!\x00");
            pushMessage((uint8*)message, strlen((char *)message));
        } else {
            UART_PutString("CORRECC!!!!\x00");
